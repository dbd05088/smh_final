...
class Finetune:
    ...
    def set_current_dataset(self, train_datalist, test_datalist, pseudo_images, pseudo_labels):
    ...
    def before_task(self, labeled_datalist, unlabeled_datalist, cur_iter, init_model=False, init_opt=True):
        logger.info("Apply before_task")
        # 새롭게 들어오는 data stream에서 class 추출
        # unlabeled datalist에 어차피 해당 task의 klass가 다 들어있음
        incoming_classes = pd.DataFrame(unlabeled_datalist)[
            "klass"].unique().tolist()

        # 기존에 있던 class와 새롭게 들어올 data의 class를 합쳐서 unique하게 추출
        self.exposed_classes = list(
            set(self.learned_classes + incoming_classes))
        ...
        if self.mem_manage == "prototype":
            self.model.fc = nn.Linear(
                self.model.fc.in_features, self.feature_size)
        ...
        logger.info(
            f"Increasing the head of fc {out_features} -> {new_out_features}")
    ...
    def update_memory(self, cur_iter, num_class=None):
        ...
        if not self.already_mem_update:
            logger.info(
                # episodic memory manage 방법
                f"Update memory over {num_class} classes by {self.mem_manage}")

            # memory에 저장할 대상 data
            #print("stream*************")
            #print(self.streamed_list)
            #print("type",type(self.streamed_list))
            #print(self.streamed_list["klass"])
            #print("stream****************")
            #print("memory*************")
            ...
            list_stream = []
            for i in range(len(self.streamed_list)):
                list_stream.append(dict(self.streamed_list.iloc[i]))
            self.streamed_list = list_stream

            # self.memory_list = list_memory
            ...
            logger.warning(
                f"Already updated the memory during this iter ({cur_iter})")

    def get_dataloader2(self, batch_size, n_worker, train_list, test_list):
        # Loader
        train_loader = None
        test_loader = None
    ...
    def train(self, cur_iter, n_epoch, batch_size, n_worker, n_passes=1):
        ...
        for epoch in range(n_epoch):
            ...
            print(f"task{cur_iter}/train/acc", train_acc, epoch)
    ...
    def _train(
        ...
        for i, data in enumerate(train_loader):
            for pass_ in range(n_passes):
                ...
                if do_cutmix:
                    x, labels_a, labels_b, lam = cutmix_data(
                        x=x, y=y, alpha=1.0)
                    logit = self.model(x)
    ...
    def evaluation_ext(self, test_list):
        ...
        test_loader = DataLoader(
            test_dataset, shuffle=False, batch_size=64, num_workers=4
        )
            ...
            with torch.no_grad():
                for data in cls_samples:
                    image = PIL.Image.open(
                        os.path.join("dataset", self.dataset,
                                     data["file_name"])
                    ).convert("RGB")
                    x = self.test_transform(image).to(self.device)
                    feature = (
                        self.feature_extractor(
                            x.unsqueeze(0)).detach().cpu().numpy()
                    )
                    feature = feature / \
                        np.linalg.norm(feature, axis=1)  # Normalize
    ...
    def uncertainty_sampling(self, samples, num_class):
        ...
        for i in range(num_class):
            ...
            if len(cls_df) <= mem_per_cls:
                # sorting할 필요도 없음
                # class당 할당된 memory보다도 적기 때문에 다 넣어주면 됨

                # records option은 각 행이 사전으로 포함됨
                '''
                ...
                # mem_per_cls보다 더 많다면, 일부 뽑아내서 sorting 해야함 (이때, jump_idx 간격으로 jump하며 추출해냄)
                jump_idx = len(cls_df) // mem_per_cls

                # uncertainty 기준 정렬 및 jump idx간격만큼 뛰며 뽑기
                uncertain_samples = cls_df.sort_values(
                    by="uncertainty")[::jump_idx]
                ret += uncertain_samples[:mem_per_cls].to_dict(
                    orient="records")

        # 빈공간이 있다면, 각 class별로 동등하게 나누어서 채워넣어주자.
        num_rest_slots = self.memory_size - len(ret)
        if num_rest_slots > 0:
            logger.warning(
                "Fill the unused slots by breaking the equilibrium.")
            ret += (
                sample_df[~sample_df.file_name.isin(
                    pd.DataFrame(ret).file_name)]
                .sample(n=num_rest_slots)
                .to_dict(orient="records")
            )

        # duplicate 함수는 중복된 항에는 true, 처음 등장하는 항에는 false를 반환
        num_dups = pd.DataFrame(ret).file_name.duplicated().sum()
        if num_dups > 0:
            logger.warning(f"Duplicated samples in memory: {num_dups}")

        # sample된 data가 class별로 labeling되어 memory에 담겨 있고, 이 memory를 return
        return ret
    ...
    def _compute_uncert(self, infer_list, infer_transform, uncert_name):
        ...
        infer_loader = DataLoader(
            infer_dataset, shuffle=False, batch_size=batch_size, num_workers=4
        )
    ...
    def montecarlo(self, candidates, uncert_metric="vr"):
        ...
        for idx, tr in enumerate(transform_cands):
            ...
            self._compute_uncert(
                candidates, _tr, uncert_name=f"uncert_{str(idx)}")
    ...
    def variance_ratio(self, sample, cand_length):
        ...
        for i in range(cand_length):
            # uncert argmin.
            top_class = int(torch.argmin(sample[f"uncert_{i}"]))
