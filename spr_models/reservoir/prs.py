    ...
    def update(self, imgs, cats, **other_datas):
        ...
        for s_i in range(nbatch):
            ...
            if self.n < self.rsvr_total_size:
                ...
                tf = self.rsvr['corrupts'].tolist()
                if not -1 in tf:
                    count_true = tf.count(1)
                    count_false = tf.count(0)
                    pure_percent = (100*count_false)/(count_true+count_false)
                    print("before", pure_percent)
