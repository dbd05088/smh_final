...
if [ "$DATASET" == "mnist" ]; then
    ...
    TOTAL=50000 N_VAL=250 N_CLASS=10 TOPK=1
    MODEL_NAME="resnet18"
    N_EPOCH=5; BATCHSIZE=128; LR=0.05 OPT_NAME="sgd" SCHED_NAME="cos"
    if [ "${MODE_LIST[0]}" == "joint" ]; then
        N_INIT_CLS=10 N_CLS_A_TASK=10 N_TASKS=1
    elif [[ "$EXP" == *"blurry"* ]]; then
        N_INIT_CLS=10 N_CLS_A_TASK=2 N_TASKS=5
...
CUDA_VISIBLE_DEVICES=5 nohup python main.py --mode $MODE --mem_manage $MEM_MANAGE --exp_name $EXP \
--dataset $DATASET \
...
--feature_size $FEAT_SIZE $distilling --joint_acc $JOINT_ACC --seed 2 \
    ...
    --log-dir logs \
    --configs configs/cifar10_spr.yaml \
    --eps episodes/cifar10-split_epc1_a.yaml \
    --override "corruption_percent=0.4" \
    --amp &
